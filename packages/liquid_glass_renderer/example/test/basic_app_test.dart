...
void main() {
  testWidgets('showcase includes one primary-tinted glass shape', (
    tester,
  ) async {
    ...
    final neutral = tester.widget<LiquidGlass>(
      find.byKey(const ValueKey('neutral-demo-glass')),
    );
    final primary = tester.widget<LiquidGlass>(
      find.byKey(const ValueKey('pill-demo-glass')),
    );
    expect(neutral.appearance, isNull);
    final tint = primary.appearance!.tint;
    expect(tint.a, greaterThan(0));
    expect(tint.b, greaterThan(tint.r));
  });

  testWidgets('each showcase shape fades independently', (tester) async {
    await tester.pumpWidget(
      const CupertinoApp(home: BasicApp(backgroundOverride: 'grid')),
    );
    await tester.pump();

    double visibility(String key) => tester
        .widget<LiquidGlassVisibility>(find.byKey(ValueKey(key)))
        .visibility;

    void tapTarget(String key) {
      tester.widget<GestureDetector>(find.byKey(ValueKey(key))).onTap!();
    }

    tapTarget('neutral-demo-tap-target');
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 100));
    expect(visibility('neutral-demo-visibility'), lessThan(1));
    expect(visibility('primary-demo-visibility'), 1);
    expect(visibility('pill-demo-visibility'), 1);

    tapTarget('primary-demo-tap-target');
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 100));
    expect(visibility('primary-demo-visibility'), lessThan(1));
    expect(visibility('pill-demo-visibility'), 1);

    tapTarget('pill-demo-tap-target');
    await tester.pump();
    await tester.pump(const Duration(milliseconds: 100));
    expect(visibility('pill-demo-visibility'), lessThan(1));
  });

  testWidgets('a fading fake showcase shape keeps its glass surface', (
    tester,
  ) async {
    await tester.pumpWidget(
      const CupertinoApp(home: BasicApp(backgroundOverride: 'grid')),
    ...
    tester
        .widget<GestureDetector>(
          find.byKey(const ValueKey('neutral-demo-tap-target')),
        )
        .onTap!();

    RenderFakeGlass? fading;
    for (var frame = 0; frame < 60; frame++) {
      await tester.pump(const Duration(milliseconds: 1));
      final fadingCandidates = tester.allRenderObjects
          .whereType<RenderFakeGlass>()
          .where(
            (shape) =>
                shape.appearance.visibility > 0 &&
                shape.appearance.visibility < 1,
          )
          .toSet();
      if (fadingCandidates.isNotEmpty &&
          fadingCandidates.single.surfaceShader != null) {
        fading = fadingCandidates.single;
        break;
      }
    }

    expect(fading, isNotNull);
    expect(fading!.backdropHandledByLayer, isFalse);
  ...
  setUp(() {
    ...
    appearanceNotifier.value = const LiquidGlassAppearance.ios27ToolbarLight();
  ...
  testWidgets('example follows platform dark appearance', (tester) async {
    ...
    expect(find.byType(LiquidGlassBlendGroup), findsNWidgets(2));
  });
  ...
  test('scalar YAML presets round-trip settings and appearance', () {
  ...
  test('bundled seed presets are available to the persistent store', () {
    ...
    expect(
      PresetStore.fromYaml(toolbar).settings.toJson(),
      const LiquidGlassSettings.ios27ToolbarLight().toJson(),
    );
    expect(
      PresetStore.fromYaml(toolbar).appearance.toJson(),
      const LiquidGlassAppearance.ios27ToolbarLight().toJson(),
    );
    expect(
      PresetStore.fromYaml(darkToolbar).settings.toJson(),
      const LiquidGlassSettings.ios27ToolbarDark().toJson(),
    );
    expect(
      PresetStore.fromYaml(darkToolbar).appearance.toJson(),
      const LiquidGlassAppearance.ios27ToolbarDark().toJson(),
    );
  });
}
