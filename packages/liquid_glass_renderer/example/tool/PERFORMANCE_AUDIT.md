...
`BackdropFilter` costs ~115 mW just for Impeller's full-screen readback. A
plain `BackdropFilter(blur σ7)` is ≈165 mW/element, FakeGlass ≈230, real
glass ≈335. Layers that share one `BackdropGroup` / `BackdropKey` pay the
readback once (two real layers 797 → 688 mW; two plain blurs 426 → 312).
Glass shadows add ~75 mW per shape (`saveLayer` + punch). Continuously
animating blend-group geometry costs ~120 mW CPU and can miss 120 Hz.
FakeGlass is not a power tier on Android (same readback and blur as real
glass); the only real tier is not sampling the backdrop at all. Frost
sigma matters: σ≤4 disables Impeller's downsample and costs more than σ7;
σ20 costs ~3× σ7. Nested blur→shader backdrop passes were tried and
rejected (−5% real / −9% fake on the GPU rail) because they need a
destructive `BlendMode.src` inner pass that breaks fade compositing and
removes the shader's ownership of the silhouette. ClickUp Inbox scroll:
glass OFF 88 / FAKE 224 / REAL 247 mW GPU, with CPU ≈1.34 W in every
state.

Two further package-side avenues were measured and closed on 2026-09-14:

- Final-shader ALU (mediump colour path, single backdrop read for sub-half-
  pixel chromatic aberration on high-DPR devices, `pow` avoidance) moved
  every scenario by less than run noise (`appScrollRealNoFrost` 417 → 414,
  `appScrollRealTopOnly` 427 → 413, `realToolbarMaterial` 209 → 208 mW).
  The PowerVR compiler already folds this; the shader is not a lever.
- The per-frame geometry rebuild (`appScrollRealTabs`, ~117 fps) spends its
  extra ~2 ms in `PAINT` on the UI thread: ~1.75 ms is the Flutter GPU
  command-buffer encode + submit and ~0.3–1.0 ms the immutable matte
  texture allocation. All Dart-side work (shape packing, geometry caches,
  filter creation) is ~0.2 ms. Pooling the matte is excluded by the
  frame-ownership contract, so this cost is engine-side.
