...
The `material_tint_*` scenes isolate SwiftUI
`.glassEffect(.regular.tint(...))` on seven solid backdrops. The checked
`ios27-iphone17pro-tint-model-v1` corpus covers blue and orange opaque tints in
light and dark appearance, plus a 40% blue tint. The measurements establish two
properties used by `LiquidGlassColorModel.ios27`:

1. An opaque tint is mapped to a backdrop-luminance-conditioned range of tones;
   it is not a flat source-over color.
2. Tint opacity linearly interpolates from Apple's untinted regular material to
   that opaque tonal result, within 8-bit capture rounding.

Flutter evaluates the fitted tonal curve in the existing final fragment pass,
using the already sampled backdrop. It adds no texture read or rendering pass.
Use the `tint-model-*-light.json` settings with the regular-material appearance,
not the toolbar appearance: the two neutral base transmissions are fitted
separately. `solid_color_atlas.py` writes the explicitly labelled Apple / Flutter
/ signed-residual comparison used for visual review.
