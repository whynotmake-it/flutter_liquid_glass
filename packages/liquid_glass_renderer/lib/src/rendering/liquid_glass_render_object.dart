...
abstract class LiquidGlassRenderObject extends RenderProxyBox {
  ...
  void _updateShaderSettings() {
    ...
    updateCommonUniforms(tintRenderShader);
  }

  List<double> _appearanceLookupData(
    List<LiquidGlassAppearance> appearances,
  ) {
    final fallback = _uniformAppearance ?? defaultAppearance;
  ...
  void _setShapeAppearances(List<LiquidGlassAppearance> appearances) {
    ...
    final uniformAppearance = usesTintOnlyAppearance || !usesShapeAppearances
        ? first
        : null;
  ...
  void paint(PaintingContext context, Offset offset) {
    ...
    if (usedShapeAppearances != _usesShapeAppearances ||
      ...
      // The contributor attachment is part of the Flutter-GPU render target.
      // Appearance values themselves are final-pass uniforms and do not
      // invalidate the SDF/material ownership textures.
      needsGeometryUpdate = true;
    }
  ...
  void _rememberEncodedGeometry(Rect bounds) {
    ...
    final sharedLength = min(
      _encodedGeometryInputs.length,
      _shapesWithGeometry.length,
    );
    for (var index = 0; index < sharedLength; index++) {
      final current = _shapesWithGeometry[index];
      _encodedGeometryInputs[index].update(
        current.$1,
        current.$2.matteRevision,
        current.$3,
      );
    }
    if (_encodedGeometryInputs.length > _shapesWithGeometry.length) {
      _encodedGeometryInputs.removeRange(
        _shapesWithGeometry.length,
        _encodedGeometryInputs.length,
      );
    }
    for (
      var index = _encodedGeometryInputs.length;
      index < _shapesWithGeometry.length;
      index++
    ) {
      final current = _shapesWithGeometry[index];
      _encodedGeometryInputs.add(
  ...
  (ui.Image, Rect) _buildGpuGeometryImage(
    ...
    try {
      ...
      final result = renderer.render(
        ...
        writeTintOnly: _usesTintOnlyAppearance,
        appearanceData: _usesShapeAppearances
